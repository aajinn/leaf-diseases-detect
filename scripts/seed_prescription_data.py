    ...
    try:
        ...
        for i in range(count):
            ...
            priority = random.choices(
                PRIORITIES, weights=[0.2, 0.3, 0.4, 0.1], k=1  # urgent, high, moderate, low
            )[0]

            # Random status (weighted towards active)
            status = random.choices(
                STATUSES, weights=[0.7, 0.2, 0.1], k=1  # active, completed, expired
            )[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("action", choices=["generate", "clear", "stats"], help="Action to perform")
    parser.add_argument(
        "--count", type=int, default=50, help="Number of prescriptions to generate (default: 50)"
    )
