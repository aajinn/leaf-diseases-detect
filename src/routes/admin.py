...
from src.database.admin_models import APIUsageRecord, APIConfig, UsageStats, UserStats
from src.database.connection import MongoDB, USERS_COLLECTION, ANALYSIS_COLLECTION
...
async def get_overview_stats(
    ...
    try:
        ...
        groq_cost = sum(record.get("estimated_cost", 0.0) for record in usage_records if record.get("api_type") == "groq")
        perplexity_cost = sum(record.get("estimated_cost", 0.0) for record in usage_records if record.get("api_type") == "perplexity")
...
async def get_all_users(
    ...
    try:
        ...
        for user in users:
            ...
            diseases = await analysis_collection.count_documents({"user_id": user_id, "disease_detected": True})
            healthy = await analysis_collection.count_documents({"user_id": user_id, "disease_detected": False})
            ...
            user_stats.append({
                ...
                "last_activity": last_analysis["analysis_timestamp"] if last_analysis else None
            })
...
async def get_api_usage(
    ...
    try:
        ...
        # Get records
        records = await usage_collection.find(query).sort("timestamp", -1).limit(100).to_list(length=100)
...
async def get_api_config(
    ...
    try:
        return {
            "groq": {
                "api_key": os.getenv("GROQ_API_KEY", "")[:20] + "..." if os.getenv("GROQ_API_KEY") else "Not set",
                "model": os.getenv("MODEL_NAME", "meta-llama/llama-4-scout-17b-16e-instruct"),
                "is_active": bool(os.getenv("GROQ_API_KEY"))
            },
            "perplexity": {
                "api_key": os.getenv("PERPLEXITY_API_KEY", "")[:20] + "..." if os.getenv("PERPLEXITY_API_KEY") else "Not set",
                "model": "sonar",
